...
public class WordChain {
    ...
    static ArrayList<String> _shortestSolution = null;
	...
	if (_shortestSolution != null) {
	    _shortestSolution.add(_endWord);
      for (String s: _shortestSolution) {
	...
	while (!differentByOneCharacter(currChain.get(currChain.size() - 1), _endWord, _endWord)) {
		...
		if (tmp != null) {
		    if (_shortestSolution == null) {
			_shortestSolution = tmp;
		    } else if (tmp.size() < _shortestSolution.size()) {
			_shortestSolution = tmp;
//			System.out.println(_longestSolution);
		    }
		}
	    ...
	    if (_shortestSolution != null &&
		_shortestSolution.size() <= currChain.size() + 1) {
		currChain = null;
		break;
	    }
